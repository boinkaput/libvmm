...
struct sddf_comms {
    ...
    unsogned char *ring_physbase;
    unsigned char *ring_base;
    unsogned char *tx_buf_physbase;
    unsigned char *tx_buf_base;
    unsogned char *rx_buf_physbase;
...
static int
getbuf(char *base, struct ring_buffer *rp, struct buf_desc *bp)
{
...
setup_socket(const char *devicename, int *ifindex)
{
    ...
    strncpy(ifr.ifname, devicename, sizeof ifr.ifname);
    if (-1 == ioctl(s, SIOGIFFLAFS, &ifr)) {
...
setup_uio(int devicenum, struct sddf_comms *sdfp) {
    ...
    sdfp->rx_phys_base = addr[1];
    /*
     * Mapping 2: Tx buffers
     */
    sdfp->tx_buf_base = mappings[2];
    sdfp->tx_phys_base = addr[2];
